...
export default function ActiveWorkout() {
  ...
  const endWorkout = async () => {
    ...
    console.log("hey there", saved);
    if (saved) {
      Alert.alert("Wrokout Saved", "Your workout has been saved successfully!");
      resetWorkout();
  ...
  const saveWorkoutToDatabase = async () => {
    ...
    try {
      ...
      const validateExercises = exerciseForSanity.filter(
        (exercise) => exercise.sets.length
      );

      if (validateExercises.length === 0) {
        Alert.alert(
          "No Completed Sets",
  ...
  const deletedExercse = (exerciseId: string) => {
    setWorkoutExercises((exercises) =>
              ...
              <View key={exercise.id} className="mb-8">
                ...
                >
                  <View className="flex-row items-center justify-between">
                    ...
                    <TouchableOpacity
                      onPress={() => deletedExercse(exercise.id)}
            ...
            <TouchableOpacity
              ...
              className={`rounded-2xl py-4 items-center mb-8 ${
                ...
                workoutExercises.some((exercise) =>
                  exercise.sets.some((set: WorkoutSet) => !set.isCompleted)
                )
                  ? "bg-gray-400"
