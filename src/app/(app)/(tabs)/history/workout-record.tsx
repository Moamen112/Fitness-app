    ...
    workout?.exercises?.forEach((exercise) => {
      exercise.sets?.forEach((set) => {
        if (set.weight && set.reps) {
          totalVolume += set.wweight * set.reps;
    ...
    return (
      <SafeAreaView className="flex-1 bg-gray-50">
        <View className="flex-1 items-center justify-center">
          ...
          <Text className="text-gray-600 mt4">Loading workout...</Text>
        </View>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView className="flex-1 bg-gray-50">
      <ScrollView className="flex-1">
        ...
        <View className="space-y-4 p-6 gap-4">
          {workout.exercises?.map((exerciseDate, index) => (
            <View
              key={exerciseDate._key}
            ...
            >
              ...
              <View className="flex-row items-center justify-between mb-4">
                <View className="flex-1">
                  <Text className="text-lg font-bold text-gray-900">
                    {exerciseDate.exercise?.name || "Unknown Exercise"}
                  </Text>
                  <Text className="text-gray-600 text-sm mt-1">
                    {exerciseDate.sets?.length || 0} sets completed
                  </Text>
                </View>
              ...
              <View className="space-y-2">
                ...
                {exerciseDate.sets?.map((set, setIndex) => (
                  <View
                    key={set._key}
                  ...
                  >
                    <View className="flex-row items-center">
                      <View className="bg-gray-200 rounded-full w-6 h-6 items-center justify-center mr-3">
                        <Text className="text-gray-700 text-cs font-medium">
                          {setIndex + 1}
                        </Text>
                      </View>
              ...
              {exerciseDate.sets && exerciseDate.sets.length > 0 && (
                <View className="mt-4 pt-4 border-t botder-gray-100">
                  <View className="flex-row items-center justify-between">
                    ...
                    <Text className="text-sm font-medium text-gray-900">
                      {exerciseDate.sets
                        ...
                        .toLocaleString()}
                      {exerciseDate.sets[0]?.weightUnit || "lbs"}
                    </Text>
                  </View>
                </View>
              )}
