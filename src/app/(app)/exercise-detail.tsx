...
const ExerciseDetail = () => {
  ...
  useEffect(() => {
    const fetchExercise = async () => {
      ...
      try {
        const exerciseDate = await client.fetch(singleExerciseQuery, { id });
        console.log(exerciseDate);
        setExercise(exerciseDate);
      } catch (err) {
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      ...
      <View className="absolute top-12 left-0 right-0 z-10 px-4">
        <TouchableOpacity
          onPress={() => router.back()}
          className="w-10 h-10 bg-black/20 rounded-full items-center justify-center bacdrop-blur-sm"
        >
          ...
          {(aiGuidance || aiLoading) && (
            <View className="mb-6">
              <View className="flex-row items-center mb-3">
                <Ionicons name="fitness" size={24} colors="#3B82F6" />
